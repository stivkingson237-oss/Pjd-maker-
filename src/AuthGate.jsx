...
export default function AuthGate({ children }) {
  ...
  async function signup(e) {
    ...
    const { data, error: signUpError } = await supabase.auth.signUp({
      email: cleanEmail,
      password,
      options: { data: { name: `${cleanLastName} ${cleanFirstName}`, first_name: cleanFirstName, last_name: cleanLastName }, emailRedirectTo: PRODUCTION_URL }
    });
    setLoading(false);
    if (signUpError) return setError(signUpError.message);
    if (data?.user && !data.session) {
      setCode(''); setMode('verify');
      setMessage(`Bonjour ${cleanFirstName} 👋 Un code de confirmation à ${OTP_LENGTH} chiffres a été envoyé à ${cleanEmail}.`);
    } else {
      setMessage(`Bienvenue ${cleanFirstName} ! Votre compte PJD Maker est créé.`); setMode('done');
    }
  }
  ...
  async function resendCode() {
    ...
    else { setCode(''); setMessage(`Bonjour ${firstName.trim()} 👋 Un nouveau code à ${OTP_LENGTH} chiffres a été envoyé à ${cleanEmail}. Utilisez uniquement le dernier code reçu.`); }
  }

  function captureCreateAccount(event) {
    const target = event.target?.closest?.('button'); if (!target) return;
    const text = (target.textContent || '').trim().toLowerCase();
    if (text.includes('créer un compte')) { event.preventDefault(); event.stopPropagation(); setError(''); setMessage(''); setFirstName(''); setLastName(''); setEmail(''); setPassword(''); setCode(''); setMode('signup'); setOpen(true); }
  }
