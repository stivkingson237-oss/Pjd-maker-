...
import { X, MailCheck, UserPlus, Loader2, ArrowLeft } from 'lucide-react';
...
export default function AuthGate({ children }) {
  ...
  async function signup(e) {
    ...
    if (!email.trim() || password.length < 6) { setError('Saisissez un e-mail valide et un mot de passe d’au moins 6 caractères.'); return; }
    setLoading(true);
    const { data, error: signUpError } = await supabase.auth.signUp({ email: email.trim(), password, options: { data: { name: email.trim().split('@')[0] } } });
    setLoading(false);
    if (signUpError) { setError(signUpError.message); return; }
    if (data?.user && !data.session) { setMode('verify'); setMessage(`Un code de confirmation a été envoyé à ${email.trim()}.`); }
    else { setMessage('Compte créé avec succès. Vous êtes maintenant connecté.'); }
  }
  ...
  async function verifyCode(e) {
    ...
    setLoading(false);
    if (verifyError) { setError('Code incorrect ou expiré. Demandez un nouveau code.'); return; }
    setMessage('E-mail confirmé avec succès. Votre compte PJD Maker est activé.'); setMode('done');
  }
  ...
  async function resendCode() {
    ...
    setLoading(false);
    if (resendError) setError(resendError.message); else setMessage(`Un nouveau code a été envoyé à ${email.trim()}.`);
  }

  function captureCreateAccount(event) {
    const target = event.target?.closest?.('button'); if (!target) return;
    const text = (target.textContent || '').trim().toLowerCase();
    if (text.includes('créer un compte')) { event.preventDefault(); event.stopPropagation(); setError(''); setMessage(''); setEmail(''); setPassword(''); setCode(''); setMode('signup'); setOpen(true); }
  }
    ...
    {open && <div className="modal-backdrop" role="presentation" onClick={() => setOpen(false)}>
      <div className="auth-modal" role="dialog" aria-modal="true" aria-label="Compte PJD Maker" onClick={e => e.stopPropagation()}>
        ...
        {mode === 'verify' ? <form onSubmit={verifyCode}>
          ...
          <input inputMode="numeric" autoComplete="one-time-code" maxLength={8} required value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,''))} placeholder="Code de confirmation" aria-label="Code de confirmation"/>
          <button className="primary" disabled={loading}>{loading ? <><Loader2 className="spin"/> Vérification...</> : <><MailCheck size={18}/> Confirmer le code</>}</button>
