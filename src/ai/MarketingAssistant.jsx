...
function localFallback(product,channels){
 const title=String(product?.title||product?.name||'Produit PJD Market').trim();
 const description=String(product?.description||'Découvrez ce produit disponible sur PJD Market.').trim();
 const price=Number(product?.price||0),promo=product?.promo_price==null?null:Number(product.promo_price);
 const shown=Number.isFinite(promo)&&promo>0&&promo<price?promo:price;
 const priceText=shown>0?` Prix : ${shown.toLocaleString('fr-FR')} FCFA.`:'';
 const posts={},headlines={},ctas={};
 channels.forEach(c=>{
  if(c==='WhatsApp'){headlines[c]=`✨ ${title}`;posts[c]=`Découvrez ${title}. ${description}${priceText}\n\nÉcrivez-nous pour commander ou obtenir plus d'informations.`;ctas[c]='Écrivez-nous pour commander'}
  else if(c==='Facebook'){headlines[c]=`${title} — sur PJD Market`;posts[c]=`🛍️ ${title}\n\n${description}${priceText}\n\nDécouvrez le produit sur PJD Market et contactez le vendeur pour commander.`;ctas[c]='Découvrir et commander'}
  else if(c==='Instagram'){headlines[c]=`${title} ✨`;posts[c]=`✨ ${title}\n\n${description}${priceText}\n\nDisponible sur PJD Market. Contactez le vendeur pour commander.`;ctas[c]='Découvrir le produit'}
  else if(c==='TikTok'){headlines[c]=`Tu cherches ${title} ? 👀`;posts[c]=`👀 Tu cherches ${title} ?\n\n${description}${priceText}\n\nDécouvre-le sur PJD Market et contacte le vendeur.`;ctas[c]='Découvre-le maintenant'}
  else {headlines[c]=title;posts[c]=`${title}.${priceText} ${description.slice(0,150)} Commandez via PJD Market.`;ctas[c]='Commander via PJD Market'}
 });
 return {campaign_title:`Campagne — ${title}`,strategy:'Présentez clairement le produit, son bénéfice réel et terminez par une action simple. Modifiez le contenu avant publication.',posts,headlines,ctas,hashtags:['#PJDMarket','#Shopping','#BoutiqueEnLigne','#Commerce','#Afrique']};
}
...
async function invokeAI(body){
 ...
 try{
  const response=await fetch('/api/pjd-ai-marketing',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),signal:controller.signal});
  const data=await response.json().catch(()=>({}));
  if(!response.ok)throw new Error(data?.error||`Erreur serveur (${response.status}).`);
  if(!data?.result)throw new Error('Le générateur n’a retourné aucun résultat.');
  return data;
...
export default function MarketingAssistant({products=[]}){
 ...
 const generate=async()=>{
  ...
  try{
   const data=await invokeAI({product,channels});
   setResult(data.result);
   if(data.source==='fallback'||data.warning)setWarning(data.warning||'Une campagne de secours a été utilisée.');
  }catch(e){
   setResult(localFallback(product,channels));
   setWarning('Le service IA est indisponible. Une campagne de secours a été générée pour vous permettre de continuer.');
 ...
 return <section className="pjd-ai-shell">
  ...
  {error&&<div className="pjd-ai-error">⚠️ {error}</div>}
  {warning&&<div className="pjd-ai-note" style={{marginTop:10,padding:10,borderRadius:10,background:'#fff7ed',color:'#9a3412'}}>⚠️ {warning}</div>}
