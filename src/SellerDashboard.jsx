...
import { Store, PackagePlus, ShoppingBag, Wallet, ArrowLeft, Loader2, Save, LayoutDashboard, Boxes, ClipboardList, Settings, Trash2, Eye, Pencil, X, CheckCircle2 } from 'lucide-react';
import './seller.css';
...
export default function SellerDashboard({ onBack }) {
  ...
  async function load(uid){
    ...
    setShop(s); if(s){setShopName(s.shop_name||'');setShopDescription(s.description||'');setCategory(s.category||'Applications & APK')}
    const {data:p}=await supabase.from('digital_products').select('*').eq('seller_id',uid).order('created_at',{ascending:false}); setProducts(p||[]);
    const {data:oi}=await supabase.from('order_items').select('id,order_id,name,price,quantity,seller_id,orders(status,created_at)').eq('seller_id',uid).order('id',{ascending:false}); setOrders(oi||[]);
    setLoading(false);
  }
  async function saveShop(e){
    ...
    const payload={owner_id:session.user.id,shop_name:shopName.trim(),slug,description:shopDescription.trim(),category,status:'active'};
  ...
  function resetProduct(){setTitle('');setDescription('');setPrice('');setFileUrl('');setProductCategory(category||categories[0]);setEditing(null)}
  function startEdit(p){setEditing(p);setTitle(p.title||'');setDescription(p.description||'');setPrice(p.price||'');setFileUrl(p.file_url||'');setProductCategory(p.category||categories[0]);setTab('products')}
  async function addOrUpdateProduct(e){
    ...
    const payload={seller_id:session.user.id,shop_id:shop.id,title:title.trim(),description:description.trim(),category:productCategory,price:Number(price),file_url:fileUrl.trim()||null};
  ...
  const nav=[['dashboard','Tableau de bord',LayoutDashboard],['shop','Ma boutique',Store],['products','Produits',Boxes],['orders','Commandes',ClipboardList],['finance','Finances',Wallet],['settings','Paramètres',Settings]];
  ...
  return <div className="seller-page">
    ...
    <div className="seller-layout">
      ...
      <main className="seller-main">
        ...
        {tab==='shop'&&<form className="seller-card wide-card" onSubmit={saveShop}><div className="page-title"><div><span className="eyebrow">VITRINE</span><h2>{shop?'Modifier ma boutique':'Créer ma boutique'}</h2><p>Configurez les informations visibles par vos clients.</p></div><Store size={34}/></div><div className="form-grid"><label>Nom de la boutique<input required value={shopName} onChange={e=>setShopName(e.target.value)} placeholder="Ex. PJD Digital Store"/></label><label>Catégorie<select value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select></label></div><label>Description<textarea value={shopDescription} onChange={e=>setShopDescription(e.target.value)} placeholder="Présentez votre activité, vos produits et vos services..."/></label><div className="shop-checklist"><div><CheckCircle2/> Boutique publique</div><div><CheckCircle2/> Catalogue produits</div><div><CheckCircle2/> Commandes clients</div><div><CheckCircle2/> Suivi des revenus</div></div><button className="primary" disabled={saving}><Save size={18}/>{saving?'Enregistrement...':'Enregistrer la boutique'}</button></form>}

        {tab==='products'&&<><div className="page-title"><div><span className="eyebrow">CATALOGUE</span><h2>Gestion des produits</h2><p>Ajoutez, modifiez ou retirez vos produits.</p></div></div><div className="seller-columns"><form className="seller-card" onSubmit={addOrUpdateProduct}><div className="seller-card-title"><PackagePlus/><div><h3>{editing?'Modifier le produit':'Ajouter un produit'}</h3><p>{editing?'Mettez à jour votre fiche produit.':'Les nouveaux produits sont soumis à validation.'}</p></div></div><input required value={title} onChange={e=>setTitle(e.target.value)} placeholder="Nom du produit"/><textarea required value={description} onChange={e=>setDescription(e.target.value)} placeholder="Description du produit"/><div className="seller-two"><input required type="number" min="0" value={price} onChange={e=>setPrice(e.target.value)} placeholder="Prix en FCFA"/><select value={productCategory} onChange={e=>setProductCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select></div><input value={fileUrl} onChange={e=>setFileUrl(e.target.value)} placeholder="URL sécurisée du fichier (optionnel)"/><div className="form-actions"><button className="primary" disabled={saving}><Save size={18}/>{saving?(editing?'Modification...':'Publication...'):(editing?'Enregistrer les modifications':'Publier le produit')}</button>{editing&&<button type="button" className="secondary" onClick={resetProduct}><X size={18}/> Annuler</button>}</div></form><div className="seller-card"><h3>Catalogue ({products.length})</h3>{products.length===0?<p>Aucun produit pour le moment.</p>:products.map(p=><div className="seller-row product-row" key={p.id}><div><b>{p.title}</b><small>{p.category} · {money(p.price)}</small></div><div className="row-actions"><span>{p.status||'en_attente'}</span><button title="Modifier" onClick={()=>startEdit(p)}><Pencil size={16}/></button><button title="Supprimer" onClick={()=>deleteProduct(p.id)}><Trash2 size={16}/></button></div></div>)}</div></div></>}

        {tab==='orders'&&<div className="seller-card wide-card"><div className="page-title"><div><span className="eyebrow">VENTES</span><h2>Gestion des commandes</h2><p>Préparez, suivez et mettez à jour les commandes de votre boutique.</p></div><ClipboardList size={34}/></div>{orders.length===0?<div className="empty-state"><ShoppingBag size={42}/><h3>Aucune commande</h3><p>Vos commandes apparaîtront ici dès vos premières ventes.</p></div>:orders.map(o=><div className="order-card" key={o.id}><div><b>{o.name}</b><span>Commande #{String(o.order_id).slice(0,8)} · {money(Number(o.price)*Number(o.quantity))}</span><small>{o.orders?.created_at?new Date(o.orders.created_at).toLocaleString('fr-FR'):''}</small></div><select value={o.orders?.status||'en_attente'} onChange={e=>updateOrderStatus(o.order_id,e.target.value)}><option value="en_attente">En attente</option><option value="confirmee">Confirmée</option><option value="preparation">Préparation</option><option value="expedition">Expédition</option><option value="livree">Livrée</option><option value="terminee">Terminée</option><option value="annulee">Annulée</option></select></div>)}</div>}

        {tab==='finance'&&<div className="seller-card wide-card"><div className="page-title"><div><span className="eyebrow">FINANCES</span><h2>Revenus et portefeuille</h2><p>Une vue claire de vos ventes enregistrées sur PJD Maker.</p></div><Wallet size={34}/></div><div className="finance-grid"><div><span>Chiffre d’affaires</span><strong>{money(revenue)}</strong></div><div><span>Commandes</span><strong>{orders.length}</strong></div><div><span>Articles vendus</span><strong>{orders.reduce((s,x)=>s+Number(x.quantity||0),0)}</strong></div></div><div className="notice-box">Les commissions, soldes disponibles, paiements en attente et retraits peuvent être reliés à votre système de paiement PJD Maker/Supabase lorsque les tables financières sont activées.</div></div>}

        {tab==='settings'&&<div className="seller-card wide-card"><div className="page-title"><div><span className="eyebrow">CONFIGURATION</span><h2>Paramètres de la boutique</h2><p>Gérez les règles essentielles de votre activité.</p></div><Settings size={34}/></div><div className="settings-list"><div><b>Statut de la boutique</b><span>{shop?'Active':'Non configurée'}</span></div><div><b>Validation des produits</b><span>Activée</span></div><div><b>Gestion du stock</b><span>Prévue pour chaque produit</span></div><div><b>Notifications</b><span>Commandes, ventes et avis</span></div><div><b>Livraison</b><span>Configuration selon les zones</span></div></div><button className="secondary" onClick={()=>setTab('shop')}><Store size={18}/> Modifier les informations de la boutique</button></div>}
      </main>
    </div>
  </div>;
}
