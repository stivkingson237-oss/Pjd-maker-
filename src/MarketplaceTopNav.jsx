...
export default function MarketplaceTopNav({ session, shop, onOpenAffiliate, onOpenAccount, onOpenCart, onOpenSeller, onOpenAuth }) {
  ...
  useEffect(() => {
    ...
    const added = (event) => {
      read(); setCartPulse(true); window.setTimeout(() => setCartPulse(false), 450);
  ...
  return (
    <>
      ...
      <style>{`@keyframes pjdCartPulse{0%{transform:scale(1)}35%{transform:scale(1.16)}65%{transform:scale(.94)}100%{transform:scale(1)}}@keyframes pjdBadgePop{0%{transform:scale(.4);opacity:0}70%{transform:scale(1.12);opacity:1}100%{transform:scale(1);opacity:1}}@keyframes pjdNoticeIn{from{transform:translateY(12px);opacity:0}to{transform:translateY(0);opacity:1}}.pjd-topnav{width:100%;position:relative;z-index:1000}.pjd-topnav-actions{display:flex!important;align-items:center;gap:6px;overflow-x:auto;overflow-y:hidden;white-space:nowrap;scrollbar-width:none;-webkit-overflow-scrolling:touch;padding:5px 4px}.pjd-topnav-actions::-webkit-scrollbar{display:none}.pjd-action{flex:0 0 auto!important;min-width:max-content}.pjd-shop-list{display:flex!important;align-items:center;justify-content:center;gap:6px;background:#111827!important;color:#fff!important;border:2px solid #f97316!important;border-radius:12px!important;padding:9px 13px!important;font-weight:900!important;box-shadow:0 4px 14px rgba(249,115,22,.25)}.pjd-shop-list svg{width:19px;height:19px;flex:0 0 auto}.pjd-cart-action{position:relative}.pjd-cart-action.pjd-pulse svg{animation:pjdCartPulse .45s ease}.pjd-cart-badge{position:absolute;top:-7px;right:-7px;min-width:20px;height:20px;padding:0 5px;border-radius:999px;background:#f97316;color:#fff;border:2px solid #fff;font:800 11px/16px Inter,system-ui,sans-serif;text-align:center;z-index:3;animation:pjdBadgePop .28s ease}.pjd-cart-notice{position:fixed;right:16px;bottom:18px;z-index:100000;background:#111827;color:#fff;padding:12px 16px;border-radius:14px;box-shadow:0 12px 35px rgba(0,0,0,.25);font:800 14px/1.3 Inter,system-ui,sans-serif;animation:pjdNoticeIn .25s ease}.pjd-all-shops-modal{position:fixed;inset:0;z-index:100001;overflow:auto;background:#f7f7f8}.pjd-all-shops-close{position:fixed;top:12px;right:14px;z-index:100005;width:44px;height:44px;border:0;border-radius:50%;background:#111827;color:#fff;display:grid;place-items:center;cursor:pointer;box-shadow:0 8px 25px rgba(0,0,0,.25)}@media(max-width:700px){.pjd-topnav{position:sticky;top:0;background:#fff;box-shadow:0 2px 12px rgba(0,0,0,.08)}.pjd-topnav-brand{flex:0 0 auto}.pjd-topnav-actions{width:100%;padding:7px 8px;gap:7px}.pjd-action{font-size:12px!important;padding:8px 10px!important}.pjd-shop-list{order:-1;min-width:112px!important;padding:9px 12px!important;font-size:13px!important}.pjd-shop-list span{display:inline!important}}`}</style>
      <div className="pjd-topnav">
        <div className="pjd-topnav-brand"><b>PJD</b><span>MARKET</span></div>
        <div className="pjd-topnav-actions">
          <button className="pjd-action pjd-shop-list" onClick={openAllShops} type="button" aria-label="Voir toutes les boutiques"><Store /><span>Boutiques</span></button>
          {session ? (
            <>
...
export function CreateShopFlow({ session, onDone, onClose }) {
  ...
  const finishShop = async (shopRecord) => {
    ...
    onDone?.(shopRecord); onClose?.();
  };
  useEffect(() => {
    if (!session?.user?.id) return undefined;
    let alive = true; setActiveSession(session); setBusy(true);
    supabase.from("shops").select("*").eq("owner_id", session.user.id).limit(1).maybeSingle().then(({ data, error }) => {
      if (!alive) return; setBusy(false);
      if (error) { setMsg(error.message); setMode("shop"); return; }
      if (data) { finishShop(data); return; }
      setMode("shop"); setMsg("Votre compte est connecté. Configurez votre boutique une seule fois.");
    });
    return () => { alive = false; };
  }, [session?.user?.id]);
  async function signup(event) {
    ...
    if (error) setMsg(error.message); else { setMode("verify"); setMsg("Compte créé. Entrez le code de confirmation reçu par e-mail."); }
    setBusy(false);
  }
  async function resend() {
    if (!email) return; setBusy(true);
    const { error } = await supabase.auth.resend({ type: "signup", email: email.trim().toLowerCase() });
    setMsg(error ? error.message : "Un nouveau code vient d’être envoyé."); setBusy(false);
  }
  async function verify(event) {
    event.preventDefault();
    if (code.length !== 8) { setMsg("Le code doit contenir exactement 8 chiffres."); return; }
    setBusy(true); setMsg("");
    const { data, error } = await supabase.auth.verifyOtp({ email: email.trim().toLowerCase(), token: code, type: "signup" });
    if (error) { setMsg(error.message); setBusy(false); return; }
    setActiveSession(data.session);
    if (!data.session?.user?.id) { setMsg("Compte confirmé. Connectez-vous pour continuer la création de la boutique."); setBusy(false); return; }
    const existing = await supabase.from("shops").select("*").eq("owner_id", data.session.user.id).limit(1).maybeSingle();
    if (existing.data) await finishShop(existing.data); else if (existing.error) setMsg(existing.error.message); else { setMode("shop"); setMsg("Compte confirmé. Configurez votre boutique une seule fois."); }
    setBusy(false);
  }
  async function saveShop(event) {
    event.preventDefault();
    if (!activeSession?.user?.id) { setMsg("Session utilisateur introuvable. Reconnectez-vous puis réessayez."); return; }
    if (!form.name.trim() || !form.category.trim()) { setMsg("Le nom et la catégorie de la boutique sont obligatoires."); return; }
    setBusy(true); setMsg("");
    const existing = await supabase.from("shops").select("*").eq("owner_id", activeSession.user.id).limit(1).maybeSingle();
    if (existing.error) { setMsg(existing.error.message); setBusy(false); return; }
    if (existing.data) { await finishShop(existing.data); setBusy(false); return; }
    ...
    const payload = { owner_id: activeSession.user.id, shop_name: form.name.trim(), slug, description: form.description.trim(), category: form.category.trim(), phone: form.phone.trim(), city: form.city.trim(), country: form.country.trim() || "Cameroun", status: "active" };
    const { data, error } = await supabase.from("shops").insert(payload).select().single();
    if (error) setMsg(error.message); else await finishShop(data);
    setBusy(false);
  }
  ...
  return (
    <div className="pjd-flow-backdrop"><div className="pjd-flow"><button className="pjd-flow-close" onClick={onClose} type="button" aria-label="Fermer"><X /></button>
      {mode === "signup" && <div><div className="pjd-flow-title"><UserPlus /><span>Inscription</span></div><h2>Créez votre compte</h2><p>Inscrivez-vous pour accéder à votre compte PJD Market.</p><form onSubmit={signup}><label>E-mail<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>Mot de passe<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={6} required /></label><button className="pjd-primary" disabled={busy} type="submit">Créer mon compte</button></form></div>}
      {mode === "verify" && <div><div className="pjd-flow-title"><Mail /><span>Vérification</span></div><h2>Confirmez votre e-mail</h2><p>Entrez le code à 8 chiffres reçu à <b>{email}</b>.</p><form onSubmit={verify}><input className="pjd-code" inputMode="numeric" maxLength={8} minLength={8} pattern="[0-9]{8}" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 8))} placeholder="00000000" required /><button className="pjd-primary" disabled={busy || code.length !== 8} type="submit">Continuer</button></form><button type="button" className="pjd-secondary" onClick={resend} disabled={busy}>Renvoyer le code</button></div>}
      {mode === "shop" && <div><div className="pjd-flow-title"><Store /><span>Créer ma boutique</span></div><h2>Configurez votre boutique</h2><p>Ces informations sont enregistrées une seule fois. Une boutique existante sera ouverte automatiquement.</p><form onSubmit={saveShop}><label>Nom de la boutique *<input value={form.name} onChange={(event) => update("name", event.target.value)} required /></label><label>Catégorie *<input value={form.category} onChange={(event) => update("category", event.target.value)} required /></label><label>Description<textarea value={form.description} onChange={(event) => update("description", event.target.value)} rows={3} /></label><div className="pjd-two"><label>Téléphone<input value={form.phone} onChange={(event) => update("phone", event.target.value)} /></label><label>Ville<input value={form.city} onChange={(event) => update("city", event.target.value)} /></label></div><button className="pjd-primary" disabled={busy} type="submit">Créer ma boutique</button></form></div>}
