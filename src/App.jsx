...
export default function App() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data } = await supabase
        .from('shops')
        .select('*')
        .eq('owner_id', accountSession.user.id)
        .maybeSingle();
      setShop(data || null);
    };
    load();
    ...
    const loadProductFromUrl = async () => {
      ...
      const [digitalResult, physicalResult] = await Promise.all([
        supabase.from('digital_products').select(
          'id,seller_id,shop_id,title,description,category,price,promo_price,cover_image,image_url,file_type,file_url,is_free,downloads,sales,stock,status,created_at,source_type,product_type,type,seller_name,preview_url'
      ...
      const data = digitalResult.data
        ? { ...digitalResult.data, product_type: digitalResult.data.product_type || 'digital' }
        : physicalResult.data
          ? { ...physicalResult.data, product_type: 'physical', cover_image: physicalResult.data.images?.[0] || null }
          : null;
      if (data) {
  ...
  async function downloadProductByTitle(title) {
    ...
    const { data } = await supabase
      ...
      .select('id,title,file_url,is_free,price')
      .eq('title', title)
      .maybeSingle();
    if (!data?.file_url) {
      alert('Le fichier de ce produit n’est pas encore disponible.');
      return;
    }
    const { data: result, error } = await supabase.functions.invoke('download-product', {
      body: { product_id: data.id }
    });
    if (error || !result?.signed_url) {
      alert(result?.error || 'Le téléchargement n’est pas disponible.');
      return;
    }
  ...
  async function openProductByTitle(title) {
    ...
    if (data) {
      setSelectedProduct(data);
      setScreen('product');
      window.history.pushState({ productId: data.id }, '', `/produit/${encodeURIComponent(data.id)}`);
    }
  }
  ...
  const handleClick = (event) => {
    ...
    if (button && (button.textContent || '').toLowerCase().includes('télécharger gratuitement')) {
      ...
      downloadProductByTitle(button.closest('.product-card')?.querySelector('h3')?.textContent?.trim());
      return;
    }
    const productCard = event.target.closest?.('.product-card');
    if (productCard && !event.target.closest('button')) {
      openProductByTitle(productCard.querySelector('h3')?.textContent?.trim());
      return;
    }
    if (!button) return;
  ...
  function openCart() {
    window.dispatchEvent(new CustomEvent('pjd-open-cart'));
  }

  function openAccount(section = 'account') {
  ...
  function handleAuthSession(session) {
    setAccountSession(session);
    setAuthMode(null);
    if (session?.user) setScreen('market');
  }

  function openSeller(existingShop) {
    if (existingShop) {
      setShop(existingShop);
      setPublicShopId(existingShop.id);
  ...
  const saveAIDraft = async (data) => {
    ...
    const payload = {
      seller_id: accountSession.user.id,
      shop_id: shop.id,
      title: data.title?.trim() || 'Nouveau produit',
  ...
  const renderSellerTab = () => {
    switch (sellerTab) {
      case 'shop':
        return <ShopManager session={accountSession} shop={shop} onBack={() => setSellerTab('dashboard')} onAddProduct={() => setShowUploader(true)} />;
      case 'products':
        return <SellerProductManager session={accountSession} onAddProduct={() => setShowUploader(true)} />;
      case 'crm':
        return <SellerCRM session={accountSession} onBack={() => setSellerTab('dashboard')} />;
      case 'orders':
        return <SellerOperations session={accountSession} initialTab="orders" onBack={() => setSellerTab('dashboard')} />;
      case 'messages':
      case 'notifications':
        return <SellerOperations session={accountSession} initialTab="notifications" onBack={() => setSellerTab('dashboard')} />;
      case 'finance':
        return <SellerDashboard session={accountSession} shop={shop} activeTab="finance" />;
      case 'stock':
        return <SellerDashboard session={accountSession} shop={shop} activeTab="stock" />;
      case 'marketing':
      case 'loyalty':
      case 'referral':
      case 'ads':
      case 'opportunities':
        return <SellerGrowthHub session={accountSession} onBack={() => setSellerTab('dashboard')} />;
      default:
        return <SellerDashboard session={accountSession} shop={shop} activeTab={sellerTab} />;
    }
  };
  ...
  const sellerSpace = (
    <div className="pjd-seller-space">
      <div className="pjd-seller-space-nav">
        <div className="pjd-seller-brand">
          <b>PJD MAKER <i>•</i><br /><span>ESPACE VENDEUR</span></b>
          <span>Gérez votre boutique et développez votre activité.</span>
        </div>
      ...
      <div className="pjd-seller-workspace">
        <SellerSpaceNav active={sellerTab} onNavigate={navigateSeller} onAddProduct={() => setShowUploader(true)} onOpenPlans={() => setScreen('seller-plans')} />
        <div className="seller-space-content">{renderSellerTab()}</div>
      </div>
    </div>
  );
  ...
  const renderScreen = () => {
    if (screen === 'product' && selectedProduct) {
      return <ProductDetail product={selectedProduct} onBack={() => { window.history.replaceState({}, '', '/'); setSelectedProduct(null); setScreen('market'); }} onBuy={(product) => { setSelectedProduct(null); setScreen('market'); window.dispatchEvent(new CustomEvent('pjd-add-to-cart', { detail: product })); }} />;
    }
    ...
    if (screen === 'admin') {
      return (
        <>
          <AdminDashboard onBack={() => setScreen('market')} />
          <button onClick={() => setScreen('promo-codes')} style={{ position: 'fixed', right: 20, bottom: 20, zIndex: 9999, padding: '13px 18px', border: 0, borderRadius: 12, background: '#f97316', color: '#fff', fontWeight: 900, cursor: 'pointer', boxShadow: '0 8px 30px rgba(0,0,0,.2)' }}>
            🏷️ Codes promo
          </button>
        </>
      );
    ...
    <AuthGate onSessionChange={handleAuthSession}>
      ...
      {screen === 'market' && (
        <MarketplaceTopNav
          ...
          onOpenAccount={(section) => section === 'favorites' ? setScreen('favorites') : openAccount(section)}
          onOpenAffiliate={() => setScreen('affiliate')}
          onOpenAuth={(mode) => setAuthMode(mode)}
        />
      )}
      <div onClickCapture={handleClick}>{renderScreen()}</div>
      {showUploader && (
        <DigitalProductUploader session={accountSession} shop={shop} onClose={() => setShowUploader(false)} onSaved={() => setShowUploader(false)} />
      )}
      {showAIPublisher && (
        <PublicationAssistant
          onClose={() => setShowAIPublisher(false)}
          onPublished={async (data) => {
            try {
              const row = await saveAIDraft(data);
              alert(`Produit « ${row.title} » enregistré en brouillon dans votre catalogue.`);
            } catch (error) {
