...
function ProductCard({ product, isFavorite, onFavorite, onOpenProduct }) {
  ...
  return <article className="product-card mh-product-card" data-pjd-product-id={product.id} onClick={() => onOpenProduct?.(product)} role={onOpenProduct ? "button" : undefined} tabIndex={onOpenProduct ? 0 : undefined} onKeyDown={(e) => { if (onOpenProduct && (e.key === "Enter" || e.key === " ")) { e.preventDefault(); onOpenProduct(product); } }}>
...
function ProductPhotoCarousel({ products, onOpenProduct }) {
  ...
  return <section className="mh-product-carousel" aria-label="Produits PJD Market" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
...
function Section({ title, eyebrow, icon: Icon, products, favorites, onFavorite, onOpenProduct, onSeeAll, empty = "Aucun produit dans cette sélection." }) {
  return <section className="mh-section"><div className="mh-section-head"><div><span className="mh-eyebrow-dark">{Icon && <Icon size={14}/>} {eyebrow}</span><h2>{title}</h2></div><button className="mh-see" type="button" onClick={onSeeAll}>Voir tout <ChevronRight size={17}/></button></div>{products.length ? <div className="mh-grid">{products.slice(0, 8).map((p) => <ProductCard key={`${p.product_type}-${p.id}`} product={p} onFavorite={onFavorite} onOpenProduct={onOpenProduct} isFavorite={favorites.some((f) => f.id === p.id && f.product_type === p.product_type)}/>)}</div> : <div className="mh-empty">{empty}</div>}</section>;
}
...
export default function MarketplaceHome() {
  ...
  if (page === "category") return <main className="marketplace-home mh-category-page">
    ...
    <section className="mh-section mh-all-products-section"><div className="mh-section-head"><div><span className="mh-eyebrow-dark"><Package size={14}/> PRODUITS DISPONIBLES</span><h2>{categoryTitle}</h2></div><span className="mh-count">{searched.length} résultat{searched.length>1?"s":""}</span></div>
      ...
      {loading ? <div className="mh-empty">Chargement des produits…</div> : searched.length ? <div className="mh-grid">{searched.map((p)=><ProductCard key={`${p.product_type}-${p.id}`} product={p} onFavorite={favorite} onOpenProduct={openProduct} isFavorite={favorites.some((f)=>f.id===p.id&&f.product_type===p.product_type)}/>)}</div> : <div className="mh-empty">Aucun produit disponible dans cette catégorie.</div>}
    </section>
  </main>;
