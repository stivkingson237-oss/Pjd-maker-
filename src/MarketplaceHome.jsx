...
const isDigital = (p) => {
  if (String(p?.source_type || "").toLowerCase().includes("physical")) return false;
  if (String(p?.source_type || "").toLowerCase().includes("digital")) return true;
  return Boolean(p?.file_url || p?.file_type);
};
...
export default function MarketplaceHome() {
  ...
  useEffect(() => { loadProducts(); loadShops(); const channel = supabase.channel("marketplace-products-home-v3").on("postgres_changes", { event: "*", schema: "public", table: "digital_products" }, loadProducts).subscribe(); return () => supabase.removeChannel(channel); }, []);
  useEffect(() => localStorage.setItem("pjd-favorites", JSON.stringify(favorites)), [favorites]);

  async function loadProducts() {
    setLoading(true);
    const { data, error } = await supabase.from("digital_products").select("id,seller_id,shop_id,title,description,category,price,promo_price,cover_image,image_url,file_type,file_url,preview_url,is_free,downloads,sales,stock,status,created_at,source_type").in("status", ACTIVE).order("created_at", { ascending: false });
