...
Deno.serve(async req=>{
 ...
 try{
  ...
  const ins=await supabase.from("payments").insert({user_id:a.user.id,order_id:orderId,amount,currency:"XAF",method:"notchpay",status:"pending",statut:"en_attente",tx_id:ref||null,payment_ref:orderId,item_ref:orderId,phone:p,operator:ch||"notchpay",provider:"notchpay",provider_reference:orderId,provider_transaction_id:ref||null,metadata:{provider:"notchpay",network:ch||null,authorization_url:url},raw_response:provider,updated_at:new Date().toISOString()});
  ...
  return json({success:true,pending:true,reused:false,orderId,paymentId:ref||orderId,authorization_url:url,status:provider?.status||"Accepted"});
